...
<main class="flex min-h-screen flex-col items-center p-8">
	<div class="text-center">
		<h1 class="mb-4 text-4xl font-bold transition-opacity duration-500" class:flash={searching}>
			Nightingale
		</h1>
		...
		<div class="relative w-full max-w-2xl">
			...
			{#if !searching}
				<button
					class="absolute bottom-16 right-1 h-10 rounded-lg px-1 transition-all {!fetchingPrompt
				...
				<div class="flex gap-2">
					{#if !searching}
						...
						<button
							...
							class="h-10 rounded-lg bg-[#0077b5] px-6 text-white hover:bg-[#006399]"
						>
							Match on <svg
								class="ml-1 inline-block h-5 w-5"
								xmlns="http://www.w3.org/2000/svg"
						...
						<div class="flex-grow">
                           ...
                           ${
															match.summary
																...
																.split('+')
																.filter(Boolean)
																.map((text) => `<span class="text-green-800">+${text}</span><br/>`)
																.join('') ?? ''
														}
                           ${
															match.summary
