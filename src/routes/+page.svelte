<script lang="ts">
    ...
    let matches = $state<(GetContributorsMatchResponse['matches'][number] | GetLinkedinProfilesMatchResponse['matches'][number])[]>([]);
    ...
    async function fetchMatches() {
        const response = await fetch(`/api/contributorsMatch?queryId=${queryId}&platform=${searchPlatform}`);
        const data: GetContributorsMatchResponse | GetLinkedinProfilesMatchResponse = await response.json();
...
<main class="min-h-screen flex flex-col items-center p-8">
    <div class="text-center">
        ...
        <div class="w-full max-w-2xl relative">
            <textarea
                bind:value={query}
                placeholder="Describe your mission and your ideal collaborator or next hire"
                class="w-full p-4 border rounded-lg mb-4 min-h-[180px] resize-none"
                disabled={!!queryId}
            ></textarea>
            {#if !searching}
            <button
